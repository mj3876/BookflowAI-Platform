...
| Client Secret | Key Vault `bookflow-client-secret` (`kv-bookflow01`) 에 저장 | AWS → Azure 인증 + auth-pod OIDC. 만료 1년 · 30일 Logic App 자동 rotate |
...
`entra-setup.sh` 실행 시 아래 시크릿이 `kv-bookflow01`에 자동 저장됨 (PREFIX=`bookflow01`).
